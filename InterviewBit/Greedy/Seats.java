...
without having any vacant seat between them in such a way 
that the total number of hops or jumps to move them should be minimum.
...
public class Solution 
{
    public int seats(String str) 
    {
        ...
        int median_right = x_pos.get(n/2);
        int median_left = x_pos.get((int) Math.ceil(n/2.0) - 1);
        long ans = 0;

        long count_xl = 0;
        for(int i = 0; i<median_left; i++)
        {
            if(str.charAt(i) == 'x')
            {
                count_xl++;
        ...
        long count_xr = 0;
        for(int i = median_right+1; i<str.length(); i++)
        {
            if(str.charAt(i) == 'x')
            {
                count_xr++;
                ans += i - median_right - 1;
            }
        }

        ans -= (count_xl)*(count_xl-1)/2;
        ans -= (count_xr)*(count_xr-1)/2;
        long diff = (median_right == median_left) ? 0 : median_right - median_left - 1;
        ans += diff*Math.min(count_xl+1, count_xr+1);
